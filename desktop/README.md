...
| --- | --- | --- | --- | --- |
| 〜1.5 分 | — | — | 正常 | 何もしない |
| **1.5 分〜** | **見ていない** | 応答なし / 置かれたまま | **サボり確定** | **Mac のカメラで顔を撮る** → 音楽を止めて説教 → Discord |
| **1.5 分〜** | **見ていない** | 操作中 | **サボり確定** | **iPhone のスクショを撮る** → 音楽を止めて説教 → Discord |
...
**画面を見ていないと確認できたら、5 分待たずに 1.5 分で確定する。** 見ていないことが
分かっているなら待つ理由がない。逆に**視線が「不明」のときは早期確定しない** ——
カメラが使えないだけで撮られては困る。
...
- 無操作が **1 分**を超えてから覗き始める
- 覗く間隔は **30 秒**。毎ティック覗くとランプが点滅し続ける
- 触り始めたら覚えていた視線を捨てる（席に戻ったのに、さっきの「見ていない」で撮られないように）
- 顔が取れない / 目が閉じている / 横を向いている → 見ていない
- 顔が正面を向いて目が開いている → 見ている
...
| `notLookingConfirmSeconds` | 90 | 画面を見ていないと分かっている場合の確定 |
| `gazeCheckSeconds` | 60 | ここまではカメラを起動しない |
| `gazeCheckIntervalSeconds` | 30 | 視線を覗く間隔 |
| `gazeFreshnessSeconds` | 60 | 視線の見立てを信じる期間 |
...
| 視線の判定 | いま画面を見ていると判定されるか（判定の入力そのもの） |
