...
- **プレビュー**: `$MIHARI_ROOM_ROOT/previews/<内容 token>/...` に不変スナップショットを置き、
  **API とは別ホスト**（`MIHARI_PREVIEW_BASE_URL`）から静的配信する想定。
  ...
  - **文書成果物**: `output/` 配下（research/ を除く）の `*.md`・`*.pdf` も同じ版に
    ...
    ダウンロードとも版の公開状態に乗せる（公開中は共有 token 経路、非公開は
    認証付き `view_url`）。文書だけのジョブは一覧の index.html を自動生成する
  - manifest（id/version/public/private 状態・sha256/source_ids/session_id/documents）は
    `root/registry/` に置き、HTTP では出さない。共有 token → 版の対応も
    `root/registry/publications.json` に置く。sha256 は決定的
