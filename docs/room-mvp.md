...
- 本番 Python は **3.13 系に固定**すること。Hermes は
  `requires-python = ">=3.11,<3.14"` のため 3.14 では本番起動しない
 （`room/deploy/README.md` の手順どおり `uv sync --python 3.13`）。
  手元の room venv が 3.14 でもテストは通るが、本番一致ではない
...
- [x] 本番 Python pin の文書化（3.13、`room/deploy/README.md`）
